  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      $.ajax( {
        ...
        if( isNaN(centerlat) ) { centerlat = -5.5111885;}
        if( isNaN(centerlng) ) { centerlng = 105.661534;}
      ...
      var map_data ={
		<?php 
			$mySql= mysql_query("Select * From lokasi") or die ();
			while ($p = mysql_fetch_array($mySql)) {
		?>
        "<?php echo $p['id']; ?>":{"lat":"<?php echo $p['lat']; ?>", "lng":"<?php echo $p['lon']; ?>", "mapinfo":"<?php echo $p['unit_kerja']; ?>"},
		<?php
			}
		?>
